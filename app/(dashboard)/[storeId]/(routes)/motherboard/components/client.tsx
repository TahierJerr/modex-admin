...
export const MotherboardClient: React.FC<MotherboardClientProps> = ({
    ...
    const handleRequest = async (skipDateCheck: boolean = false) => {
        ...
        try {
            setLoading(true);
            await axios.get(`/api/${params.storeId}/updatePrices?skipDateCheck=${skipDateCheck}&productModel=motherboard`);
    ...
    <div className="flex items-center justify-between">
        <Heading 
        title={`motherboard (${data.length})`}
        description="Manage motherboard for your store"
        />
    ...
    <Heading title="API" description="API calls for motherboard"/>
